%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Specular Line- Diffuse Pin Model-L11-5v- Plane Wave Transmission
%  Date 22-10-2021
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;
clear all;
...
%addpath(genpath(pwd));
filepath = pwd;
experiment = 'Cyst_No_lens';
dataset_owner = 'Hari';
dataset_name = 'No_lens';
Fnum = 1.5;

GPU             = true;   % set to true to simulate using GPU, to false to simulate in CPU
USE_CPU_C_CODE  = true;  % set to true to simulate using CPU cores in the absence of CUDA
...
Trans.tx_pw_steering_angles = [-Trans.tx_steering_angles_start:(2*Trans.tx_steering_angles_start)/(Trans.tx_steering_angles-1):Trans.tx_steering_angles_start];           % Steering angle of the transmit signal [degrees]
...
%% object 
x_pos_mat=[40e-3];%Nx*dx/2+10e-3];%[20e-3 : 5e-3 : 40e-3];
y_pos_mat=[Ny*dy/2];%[-10e-3:5e-3:10e-3]%
radius =0.01; 
...
medium.density(scattering_region2 == 1) = scattering_rho0(scattering_region2 == 1);
 ...
 %% LENS
% elem=int16(16*(0.3e-3)/dx); 
% width=int16(128*(0.3e-3)/dx); %transducer width
% x=zeros(1,elem);
% y=zeros(1,elem);
% x=-2.5:(2*2.5)/(length(x)-1):2.5;
% % x_position=((Ny/2 - width/2):1:(Ny/2 + width/2));
% for a=1:1:elem
%     y(a)=-sqrt(2.5*2.5-x(a)*x(a))+10; % for concave lens
% end
% x_dx=x.*((1e-3)/dx);
% y_dy=round(y.*((1e-3)/dy));
% % big lens
% x1=zeros(1,width);
% y1=zeros(1,width);
% x1=-19.2:(2*19.2)/(length(x1)-1):19.2;
% for a=1:1:width
%     y1(a)=-sqrt(53.88*53.88-x1(a)*x1(a))+63.88; % for concave lens
% end
% x1_dx=x1.*((1e-3)/dx);
% y1_dy=round(y1.*((1e-3)/dy));
% Lens=zeros(1,width);
% %% total lens
% % y1_dy(1,length(y1_dy)/2-length(y_dy)/2:length(y1_dy)/2+length(y_dy)/2-1)=y_dy;
% %
% i=int16(Ny/2 - width/2);
% 
% % Big lens medium properties - Acoustic GEL
% for j=1:1:length(y1_dy)
%      medium.sound_speed(1:y1_dy(j),i, :) = 1640;
%      medium.density(1:y1_dy(j),i, :) = 1040.00;
%      i=i+1;
% end
% small_lens_bottom=min(y_dy');
% small_lens_top=max(y_dy');
% i=int16(Ny/2 - elem/2);
% 
