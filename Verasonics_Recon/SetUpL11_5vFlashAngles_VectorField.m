...
Resource.RcvBuffer(1).rowsPerFrame = na*2048*2; % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 2;    % 10 frames stored in RcvBuffer.
...
Process(2).method = 'Reconstruct_Vector_Field';
Process(2).Parameters = {'srcbuffer','receive',...  % buffer to process, in this case InterBuffer
    'srcbufnum',1,...
    'srcframenum',1,...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    nsc = nsc+1;
     Event(n).info = 'recon with recon software';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 1;
    Event(n).process = 1;
...
%     Event(n).info = 'Reconstruct with External Func';
%     Event(n).tx = 0;
%     Event(n).rcv = 0;
%     Event(n).recon = 0;
%     Event(n).process = 2;
%     Event(n).seqControl = 0;
    ...
    if floor(i/2) == i/2     % Exit to Matlab every 5th frame
